  ...
  const navigationGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { name: "Performance Dashboard", href: "/performance", icon: TrendingUp, roles: ["superadmin", "admin"] },
      ]
    },
    {
      name: "Personal",
  ...
  return (
    <>
      ...
      >
        ...
        {/* Logo Section */}
        <div className={`${isCollapsed && !isHovered ? 'p-3' : 'p-6'} border-b border-slate-200 transition-all duration-300`}>
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-primary rounded-lg flex items-center justify-center flex-shrink-0">
              <Heart className="text-white h-6 w-6" />
            </div>
            <div className={`transition-all duration-300 ${isCollapsed && !isHovered ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'}`}>
          ...
          {/* Quick Search - Only show when not collapsed or when hovering */}
        ...
        {/* Navigation Menu */}
        <nav className={`flex-1 ${isCollapsed && !isHovered ? 'p-2' : 'p-4'} transition-all duration-300 overflow-y-auto`}>
              ...
              <div key={group.name}>
                ...
                >
                  <CollapsibleTrigger asChild>
                    <button 
                      className={`w-full flex items-center ${isCollapsed && !isHovered ? 'justify-center' : 'justify-between'} px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-lg transition-all duration-200 group relative`}
                      onClick={(e) => {
                        e.preventDefault();
                        toggleGroup(group.name);
                      }}
                    >
                      <div className={`flex items-center ${isCollapsed && !isHovered ? 'justify-center' : 'space-x-2'}`}>
                        <group.icon className="w-4 h-4 flex-shrink-0" />
                        <span className={`transition-all duration-300 ${isCollapsed && !isHovered ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'}`}>
                          {group.name}
                        </span>
                      </div>
                      {(!isCollapsed || isHovered) && (
                      ...
                      {/* Tooltip for collapsed state without hover */}
                      {isCollapsed && !isHovered && (
                  ...
                  <CollapsibleContent className={`${isCollapsed && !isHovered ? 'hidden' : 'block'}`}>
        ...
        {/* User Profile */}
        <div className={`${isCollapsed && !isHovered ? 'p-2' : 'p-4'} border-t border-slate-200 transition-all duration-300`}>
          <div className={`flex items-center ${isCollapsed && !isHovered ? 'justify-center' : 'space-x-3'} group relative`}>
            ...
            <div className={`flex-1 transition-all duration-300 ${isCollapsed && !isHovered ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'}`}>
          ...
          <div className={`mt-3 space-y-2 ${isCollapsed && !isHovered ? 'flex flex-col items-center' : ''}`}>
            <Button
              variant="ghost"
              size={isCollapsed && !isHovered ? "sm" : "sm"}
              onClick={handleLogout}
              className={`text-red-600 hover:text-red-700 hover:bg-red-50 transition-colors ${
                isCollapsed && !isHovered ? 'w-8 h-8 p-1' : 'w-full justify-start text-xs'
              }`}
            >
              <LogOut className="w-4 h-4" />
              {(!isCollapsed || isHovered) && <span className="ml-2">Logout</span>}
            </Button>

            {onStartTour && (
              <div className={isCollapsed && !isHovered ? 'flex flex-col items-center' : ''}>
                <Button
                  variant="ghost"
                  size="sm" 
                  onClick={onStartTour}
                ...
                >
                  ...
                  {(!isCollapsed || isHovered) && <span className="ml-2">Help & Tour</span>}
                </Button>
              </div>
            )}
          </div>
